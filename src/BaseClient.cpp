...
bool CUpDownClient::Disconnected(const wxString& strReason, bool bFromSocket)
{
        ...
        if (!bFromSocket && m_socket) {
                wxASSERT ( theApp->listensocket and theApp->listensocket->IsValidSocket ( m_socket ) );
                m_socket->Safe_Delete();
        }
...
{
        // Kad reviewed
        if ( theApp->listensocket and (theApp->listensocket->TooManySockets() && !bIgnoreMaxCon) ) {
